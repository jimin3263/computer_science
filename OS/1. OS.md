...
- 자원 사용의 주체
- 프로세스 종료 = 자원 회수
- 다수의 프로세스 존재, 하나 또는 여러 개의 CPU에서 동시에 동작 -> multplexing
...
- 모든 명령 또한 실행되기 위해 메인메모리에 적재 되어야 함 -> stored program computer
- 메인메모리 관리: 언제 어떤 내용을 메모리에 저장할 것인지 -> CPU 활용도 높이고 사용자 응답시간 개선
